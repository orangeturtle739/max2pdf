...
static int betterMain(int argc, char *argv[]) {
  ...
  err_info* e;
  Filemax in_f(in.absoluteDir().absolutePath() + "/", in.fileName(), nullptr);
  e = in_f.load();
  if (e != nullptr) {
    return 3;
  }
  // Filepdf out_f(out.absoluteDir().absolutePath(), out.baseName(), nullptr);
  Operation op("foobar", 0, nullptr);
  QString uniq = "pdf42";
  File* out_f;
  e = in_f.duplicateToDesk(nullptr, File::Type_pdf, uniq, 1, op, out_f);
  if (e == nullptr) {
    QFile temp(out_f->pathname());
    if (temp.rename(out.absoluteFilePath())) {
      return 0;
    } else {
      return 2;
    }
